...
const sendEmailPayment = async (emailDetails) => {
  try {
    ...
    const paymentTemplatePath = path.join(__dirname, "payment-template.html");
    ...
    const htmlItemRowTemplate = fs.readFileSync(itemRowTemplatePath, "utf8");

    let itemsHtml = tickets
      .map((item) => {
        return htmlItemRowTemplate
          .replace("{{ticketType}}", item.ticketType)
          .replace("{{quantity}}", String(item.quantity).padStart(2, "0"))
          .replace("{{price}}", `${item.price.toLocaleString("vi-VN")} ₫`)
          .replace(
            "{{total}}",
    ...
    htmlPaymentTemplate = htmlPaymentTemplate
      ...
      .replace("{{totalPrice}}", `${amount.toLocaleString("vi-VN")} ₫`)
