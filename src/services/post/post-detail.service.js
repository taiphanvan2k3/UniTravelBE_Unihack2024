    ...
    if (mileStones[post.upvoteCount]) {
      ...
      if (rangeOfVoucher.includes(existingMileStone.quantity)) {
        ...
        if (voucher.remaining > 0) {
          ...
          if (
            ((voucher.code === "5% off" || voucher.code === "10% off") &&
              collectedVouchers.length < 2) ||
            ((voucher.code === "15% off" || voucher.code === "20% off") &&
            ...
            voucher.remaining -= 1;
            await Promise.all([voucherForUser.save(), voucher.save()]);
          }
        }
      }
    }
  ...
  try {
    ...
    const imageUploadPromises =
      images && images.length > 0
        ? images.map((image) => uploadFileFromFilePath(image.path, bucketName))
        : [];

    const videoUploadPromises =
      videos && videos.length > 0
        ? videos.map((video) => uploadFileFromFilePath(video.path, bucketName))
        : [];
